...
const convertSDBObjToResObj = (dbObj) => {
  return {
...
const convertDDBObjToResObj = (dbObj) => {
  return {
    districtId: dbjObj.district_id,
...
app.get("/states/", authenticateToken, async (request, response) => {
  ...
  response.send(
    statesArray.map((eachState) => convertSDBObjToResObj(eachState))
  );
});
...
app.get("/states/:stateId/", authenticateToken, async (request, response) => {
  ...
  response.send(convertSDBObjToResObj(state));
});
...
app.get(
  ...
  async (request, response) => {
    ...
    response.send(convertDDBObjToResObj(district));
  }
);
...
app.get(
  ...
  async (request, response) => {
    const { stateId } = request.params;
    const getStatesStatQuery = `SELECT SUM(cases), SUM(cured), SUM(cured), SUM(deaths) FROM district WHERE state_id = ${stateId};`;
